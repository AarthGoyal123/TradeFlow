...
def test_production_settings_reject_default_secret() -> None:
    with pytest.raises(
        ValidationError, match="TRADEFLOW_AUTH_SECRET must be changed in production"
    ):
        Settings(
...
def test_production_settings_reject_short_secret() -> None:
    with pytest.raises(
        ValidationError, match="TRADEFLOW_AUTH_SECRET must be at least 32 characters long"
    ):
        Settings(
            environment="production",
...
def test_production_settings_reject_wildcard_cors() -> None:
    with pytest.raises(
        ValidationError, match="TRADEFLOW_CORS_ORIGINS must not contain '\\*' in production"
    ):
        Settings(
            environment="production",
            auth_secret="this-is-a-long-enough-secret-for-production",
...
def test_production_settings_google_oauth_requires_secret() -> None:
    with pytest.raises(
        ValidationError, match="TRADEFLOW_GOOGLE_CLIENT_SECRET is required when using Google OAuth"
    ):
        Settings(
            environment="production",
            auth_secret="this-is-a-long-enough-secret-for-production",
...
def test_production_settings_google_oauth_requires_redirect_uri() -> None:
    with pytest.raises(
        ValidationError, match="TRADEFLOW_GOOGLE_REDIRECT_URI is required when using Google OAuth"
    ):
        Settings(
            environment="production",
            auth_secret="this-is-a-long-enough-secret-for-production",
