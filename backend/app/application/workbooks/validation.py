"""Workbook validation application service."""
...
class WorkbookValidationService:
    """Validate workbook structure against a processing template."""
    ...
    def validate(self, *, template_id: str, workbook_path: Path) -> WorkbookValidationResult:
        """Validate workbook structure, worksheet selection, and required columns."""
        ...
        mapped_columns, issues = self._column_mapper.map_columns(template=template, header=header)
        return WorkbookValidationResult(
            template_id=template_id,
            workbook_path=workbook_path,
            valid=not issues,
