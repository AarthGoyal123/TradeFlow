...
class S3OutputStorage(OutputStorage):
    ...
    def save_output(self, job_id: str, output_type: OutputType, file_obj: BinaryIO) -> OutputArtifact:
    ...
    def get_output(self, job_id: str, output_type: OutputType) -> BinaryIO:
        ...
        try:
            ...
            tmp = tempfile.SpooledTemporaryFile(max_size=10_000_000, mode="w+b")
...
class S3UploadedFileStorage(UploadedFileStorage):
    ...
    def get_upload(self, stored_path: Path) -> BinaryIO:
        ...
        try:
            tmp = tempfile.SpooledTemporaryFile(max_size=50_000_000, mode="w+b")
            self._s3.download_fileobj(self._bucket_name, object_key, tmp)
            tmp.seek(0)
            return tmp
        except ClientError as exc:
